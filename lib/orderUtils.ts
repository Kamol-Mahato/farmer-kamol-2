...
export function getBangladeshDayBoundaries(forDate?: Date) {
  ...
  const startUTC =
    new Date(`${bdDateStr}T00:00:00.000Z`).getTime() - BD_OFFSET_MS;
  ...
  for (const item of orderItems) {
    ...
    if (willRestore) {
      ...
      if (!freshProduct || freshProduct.stockQty < kg) {
        throw new Error(
          `STOCK_ERROR:"${freshProduct?.name}" এর পর্যাপ্ত স্টক নেই এই অর্ডার আবার সক্রিয় করার জন্য। উপলব্ধ: ${freshProduct?.stockQty ?? 0}`,
        );
      }
      await tx.product.update({
        where: { id: item.productId },
        data: { stockQty: { decrement: kg } },
      });
    }
  }
}
...
export async function applyPartialDeliveryStockRestore(
  tx: any,
  orderId: number,
  receivedQty: number,
) {
  ...
  const totalRawQty = orderItems.reduce(
  ...
  for (const item of orderItems) {
    ...
    await tx.product.update({
      where: { id: item.productId },
      data: { stockQty: { increment: restoreKg } },
    });
  }
}
export async function resolveOrderIdFromCustomId(
  rawId: string,
): Promise<number | null> {
  ...
  const prefixPattern = new RegExp(
    `^${siteConfig.business.orderIdPrefix}(\\d{4})(\\d{2})(\\d{2})(\\d+)$`,
    "i",
  );
