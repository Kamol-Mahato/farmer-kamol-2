...
(function () {
  function S() {
    return window.__fkGet ? window.__fkGet() : null;
  }
  function set(p) {
    if (window.__fkSet) window.__fkSet(p);
  }
  ...
  window.duplicateElement = function () {
    ...
    if (!el) {
      alert("আগে একটি লেয়ার সিলেক্ট করুন");
      return;
    }
  ...
  async function serializeDesign() {
    ...
    if (s.bgData && s.bgData.image)
      bgImageData = await imgToDataURL(s.bgData.image);
    return {
      version: 1,
      CW: s.CW,
      CH: s.CH,
      ...
      bgMode: s.bgMode,
      bgData: {
        solid: s.bgData.solid,
        grad1: s.bgData.grad1,
        grad2: s.bgData.grad2,
        grad3: s.bgData.grad3,
        useGrad3: s.bgData.useGrad3,
        angle: s.bgData.angle,
        opacity: s.bgData.opacity,
        blur: s.bgData.blur,
  ...
  async function applyDesign(data) {
    const s = S();
    if (!s || !data || !data.elements) return;
    if (data.CW && data.CH) {
      set({
        CW: data.CW,
        CH: data.CH,
        currentRatio: data.currentRatio || s.currentRatio,
      });
      const wrap = document.getElementById("canvasWrap");
      if (wrap) {
        wrap.style.width = data.CW + "px";
        wrap.style.height = data.CH + "px";
      }
    ...
    if (data.bgData) {
      Object.assign(s.bgData, {
        solid: data.bgData.solid,
        grad1: data.bgData.grad1,
        grad2: data.bgData.grad2,
        grad3: data.bgData.grad3,
        useGrad3: data.bgData.useGrad3,
        angle: data.bgData.angle,
        opacity: data.bgData.opacity,
        blur: data.bgData.blur,
      });
      if (data.bgData._imageData)
        s.bgData.image = await loadImageFromData(data.bgData._imageData);
    }
    ...
    for (const el of data.elements) {
      const copy = Object.assign({}, el);
      if (copy._imgData) {
        copy.img = await loadImageFromData(copy._imgData);
        delete copy._imgData;
      }
  ...
  function setSavedTemplates(list) {
  ...
  window.loadTemplateById = async function (id) {
    const t = getSavedTemplates().find(function (x) {
      return x.id === id;
    });
    if (!t) return;
  ...
  window.deleteTemplateById = function (id) {
    setSavedTemplates(
      getSavedTemplates().filter(function (x) {
        return x.id !== id;
      }),
    );
    refreshTemplateList();
  ...
  function refreshTemplateList() {
    ...
    if (!list.length) {
      box.innerHTML =
        '<div style="color:var(--text2);font-size:12px;padding:12px;text-align:center;">কোনো সেভ করা টেমপ্লেট নেই</div>';
      return;
    }
    box.innerHTML = list
      .map(function (t) {
        return (
          ...
          new Date(t.savedAt).toLocaleString("bn-BD") +
          "</div></div>" +
          '<button class="btn btn-primary" style="padding:4px 10px;font-size:11px;" onclick="loadTemplateById(' +
          t.id +
          ')">Load</button>' +
          '<button class="btn btn-ghost" style="padding:4px 8px;font-size:11px;color:#e74c3c;" onclick="deleteTemplateById(' +
          t.id +
          ')">✕</button></div>'
        );
      })
    ...
    if (box) {
      box.innerHTML = BATCH_SIZES.map(function (s, i) {
        return (
          ...
          '<input type="checkbox" class="batchSizeCb" data-idx="' +
          i +
          '" checked style="accent-color:var(--accent);"> ' +
          s.label +
  ...
  window.runBatchExport = async function () {
    const s = S();
    if (!s) return;
    const cbs = [].slice.call(
      document.querySelectorAll(".batchSizeCb:checked"),
    );
    if (!cbs.length) {
      alert("কমপক্ষে ১টি সাইজ সিলেক্ট করুন");
      return;
    }
    const format =
      (document.getElementById("batchFormat") || {}).value || "png";
    const quality = parseFloat(
      (document.getElementById("batchQuality") || {}).value || "0.92",
    );
    ...
    for (let i = 0; i < cbs.length; i++) {
      ...
      const scaled = oldEls.map(function (el) {
        ...
        e.h = (el.h || 0) * sy;
        if (el.fontSize)
          e.fontSize = Math.max(8, Math.round(el.fontSize * Math.min(sx, sy)));
  ...
  window.applyImagePreset = function (name) {
    ...
    const el = s.getSelected();
    if (!el || el.type !== "image") {
      alert("আগে একটি ইমেজ লেয়ার সিলেক্ট করুন");
      return;
    }
    const presets = {
