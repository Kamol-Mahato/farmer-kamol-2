...
export default async function FosolItemEnPage({
  ...
  const articleSchema = {
    ...
    publisher: {
      ...
      logo: { "@type": "ImageObject", url: `${siteConfig.domain.url}${siteConfig.domain.logo}` },
    },
    mainEntityOfPage: { "@type": "WebPage", "@id": pageUrl },
  }

  const related = await prisma.fosolItem.findMany({
    where: { categoryId: item.categoryId, isPublished: true, id: { not: item.id } },
    take: 4,
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 pt-8 md:pt-6">
        <nav className="text-sm text-gray-500 mb-4">
          ...
          <Link href="/en/banglar-fosol" className="hover:text-green-700">Banglar Fosol</Link>
          <span className="mx-1.5">/</span>
          <Link href={`/en/banglar-fosol/${item.category.slug}`} className="hover:text-green-700">
            {catName}
          </Link>
          <span className="mx-1.5">/</span>
        ...
        <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">{catName}</span>
        ...
        {related.length > 0 && (
          <div className="mt-12 border-t border-gray-100 pt-8">
            <h2 className="text-lg font-bold text-green-900 mb-4">More in this category</h2>
