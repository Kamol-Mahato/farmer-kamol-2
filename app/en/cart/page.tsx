  ...
  return (
    <div className="relative">
      <input
        ...
        onBlur={() => setTimeout(() => setShow(false), 200)}
        placeholder={
          disabled ? "Select district first" : "Type or select upazila"
        }
        disabled={disabled}
...
export default function CartPage() {
  ...
  function loadCart() {
    const stored = JSON.parse(
      localStorage.getItem("farmer_kamol_cart") || "[]",
    );
    setCart(stored);
    setLoaded(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isValidBDPhone(form.phone)) {
      setError(
        "Your mobile number is invalid (must be 11 digits and start with 01)",
      );
      return;
    }
    if (!form.paymentMethod) {
      setError(
        "Please select a payment method (Cash on Delivery or Online Payment)",
      );
      return;
    }
    if (form.paymentMethod === "GATEWAY") {
      if (!form.gatewayName) {
        setError(
          "Please select which method (bKash/Nagad/Rocket) you sent money through",
        );
        return;
      }
      if (!form.trxId.trim()) {
        ...
        {form.paymentMethod === "GATEWAY" && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-3 space-y-3">
            <p className="text-xs font-bold text-gray-700">
              Which method did you send money through? *
            </p>
        ...
        <div className="bg-gray-50 rounded-lg p-3 text-sm">
          ...
          <div className="flex justify-between border-t pt-1 mt-1">
            ...
            <span className="font-bold text-green-700">
              ৳ {totalProductPrice + deliveryCharge}
            </span>
          </div>
        </div>
        <button
