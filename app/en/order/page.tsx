...
function OrderForm() {
    ...
    return (
      <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-200 mb-4 text-left">
        ...
        <p className="text-gray-500 text-xs mb-3">
          Set a password to track your order later (optional)
        </p>
        <input
          type="password"
          value={password}
  ...
  async function handleSubmit(e: React.FormEvent) {
    if (!isValidBDPhone(form.phone)) {
      setError(
        "Your mobile number isn't valid (must be 11 digits and start with 01)",
      );
      return;
    }
    e.preventDefault();
    ...
    if (!form.paymentMethod) {
      setError(
        "Please choose a payment method (Cash on Delivery or Online Payment)",
      );
      return;
    }
    if (form.paymentMethod === "GATEWAY") {
      if (!form.gatewayName) {
        setError(
          "Select which method (bKash/Nagad/Rocket) you sent money with",
        );
        return;
      }
      if (!form.trxId.trim()) {
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md text-center">
          ...
          <div className="bg-green-50 rounded-xl p-4 border border-green-100 text-left mb-4">
            ...
            <p className="text-gray-500 text-xs">
              Quantity: {form.quantity} × ৳ {product?.pricePerUnit}
            </p>
            <p className="text-gray-500 text-xs">
          ...
          <div>
            ...
            {form.phone.length > 0 && !isValidBDPhone(form.phone) && (
              <p className="text-red-500 text-[10px] mt-1 font-bold">
                Enter a valid 11-digit number (starting with 01)
              </p>
            )}
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <UpazilaSearch
              ...
              upazilas={
                selectedDistrictId ? getEnglishUpazilas(selectedDistrictId) : []
              }
              value={form.upazila}
        ...
        {form.paymentMethod === "GATEWAY" && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-3 space-y-3">
            <p className="text-xs font-bold text-gray-700">
              Which method did you send money with? *
            </p>
