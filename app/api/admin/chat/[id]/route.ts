...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "চ্যাট লোড করতে সমস্যা হয়েছে" },
      { status: 500 },
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    if (!admin && existing.assignedToId && existing.assignedToId !== user.id) {
      return NextResponse.json(
        {
    ...
    if (typeof visitorName === "string")
      data.visitorName = visitorName.trim() || null;
    if (typeof visitorPhone === "string")
      data.visitorPhone = visitorPhone.trim() || null;
