...
export async function POST(
  ...
  try {
    ...
    if (order.orderStatus !== "PARTIAL_DELIVERY") {
      return NextResponse.json(
        { error: "এই অর্ডারটি Partial Delivery স্ট্যাটাসে নেই" },
        { status: 400 },
      );
    }
    if (order.receivedQty !== null && order.receivedQty !== undefined) {
      return NextResponse.json(
        { error: "এই অর্ডারের Received তথ্য আগেই দেওয়া হয়ে গেছে" },
        { status: 400 },
      );
    }
    ...
    if (isNaN(receivedQty) || receivedQty < 0 || receivedQty > totalQty) {
      return NextResponse.json(
        { error: `০ থেকে ${totalQty}-এর মধ্যে একটা সংখ্যা দিন` },
        { status: 400 },
      );
    }
