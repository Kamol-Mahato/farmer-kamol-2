    ...
    if (current.length >= MAX_HERO_VIDEOS && current[0]) {
      ...
      if (count >= MAX_HERO_VIDEOS) {
        return NextResponse.json(
          { error: `সর্বোচ্চ ${MAX_HERO_VIDEOS}টা ভিডিও রাখা যাবে` },
          { status: 400 },
        );
      }
    }
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    const target = await prisma.youtubeVideo.findUnique({
      where: { id: videoId },
    });
    ...
    const neighborOrder =
      direction === "up" ? target.heroOrder - 1 : target.heroOrder + 1;
    const neighbor = await prisma.youtubeVideo.findFirst({
      where: { heroOrder: neighborOrder },
    });
    if (!neighbor) {
    ...
    await prisma.$transaction([
      prisma.youtubeVideo.update({
        where: { id: target.id },
        data: { heroOrder: neighborOrder },
      }),
      prisma.youtubeVideo.update({
        where: { id: neighbor.id },
        data: { heroOrder: target.heroOrder },
      }),
    ]);
    revalidatePath("/");
    revalidatePath("/en");
