...
export async function PUT(
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "শিপিং চার্জ সঠিকভাবে দিন" },
        { status: 400 },
      );
    }
    ...
    if (
      authUser.role === "AGENT" &&
      TERMINAL_STATUSES.includes(existingOrder.orderStatus)
    ) {
      return NextResponse.json(
        {
          error:
            "এই অর্ডার ফাইনাল স্ট্যাটাসে আছে — শুধু Admin এখন এডিট করতে পারবে",
        },
        { status: 403 },
      );
    }
    ...
    for (const oldItem of existingOrder.orderItems) {
      ...
      const kg =
        oldItem.quantity * getUnitToKgMultiplier(oldProduct?.unit || "কেজি");
    ...
    const totalProductPrice = (items as { price: number }[]).reduce(
      (sum, item) => sum + Number(item.price || 0),
      0,
    );
    const deliveryCharge = Number(shipping);
    const finalCodAmount =
      totalProductPrice + deliveryCharge - (existingOrder.discountAmount || 0);

    // 📝 কী কী বদলালো তার সংক্ষিপ্ত সারাংশ — OrderEditLog-এ সেভ হবে
    const changes: string[] = [];
    if (existingOrder.deliveryAddress !== address)
      changes.push(`ঠিকানা: "${existingOrder.deliveryAddress}" → "${address}"`);
    if (existingOrder.totalProductPrice !== totalProductPrice)
      changes.push(
        `পণ্যমূল্য: ৳${existingOrder.totalProductPrice} → ৳${totalProductPrice}`,
      );
    if (existingOrder.deliveryCharge !== deliveryCharge)
      changes.push(
        `শিপিং: ৳${existingOrder.deliveryCharge} → ৳${deliveryCharge}`,
      );
    if (existingOrder.finalCodAmount !== finalCodAmount)
      changes.push(
        `মোট COD: ৳${existingOrder.finalCodAmount} → ৳${finalCodAmount}`,
      );
    const oldItemsSummary = existingOrder.orderItems
      .map((i) => `${i.productId}x${i.quantity}`)
      .sort()
      .join(",");
    const newItemsSummary = (items as { productId: number; quantity: number }[])
      .map((i) => `${i.productId}x${i.quantity}`)
      .sort()
      .join(",");
    if (oldItemsSummary !== newItemsSummary)
      changes.push(`পণ্য/পরিমাণ পরিবর্তিত হয়েছে`);
    const changesSummary =
      changes.length > 0
        ? changes.join(" | ")
        : "কোনো মূল্য/ঠিকানা পরিবর্তন হয়নি (শুধু নাম/ফোন/নোট সংশোধন সম্ভব)";
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const [productIdStr, delta] of Object.entries(stockDelta)) {
        ...
        if (delta > 0 && (!freshProduct || freshProduct.stockQty < delta)) {
          throw new Error(
            `STOCK_ERROR:"${freshProduct?.name}" এর পর্যাপ্ত স্টক নেই। উপলব্ধ: ${freshProduct?.stockQty ?? 0}`,
          );
        }
