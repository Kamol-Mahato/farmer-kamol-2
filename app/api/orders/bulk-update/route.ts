...
import {
  resolveOrderIdFromCustomId,
  applyStockChangeForStatusTransition,
} from "@/lib/orderUtils";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const mode: string =
      body.mode === "COURIER_PAYMENT" ? "COURIER_PAYMENT" : "STATUS";
    ...
    if (rows.length > 500) {
      return NextResponse.json(
        { error: "একসাথে সর্বোচ্চ ৫০০টি সারি আপডেট করা যাবে" },
        { status: 400 },
      );
    }
    ...
    if (mode === "COURIER_PAYMENT") {
      for (const row of rows) {
        ...
        if (
          courierPaidAmount === undefined ||
          courierPaidAmount === "" ||
          isNaN(Number(courierPaidAmount))
        ) {
          results.push({
            orderIdRaw: row.orderIdRaw,
            success: false,
            reason: "Courier Paid Amount সঠিক নয়",
          });
          continue;
        }
        ...
        if (!orderId) {
          results.push({
            orderIdRaw: row.orderIdRaw,
            success: false,
            reason: "Order ID খুঁজে পাওয়া যায়নি",
          });
          continue;
        }
        ...
        if (
          !requiresCollectedAmount(currentOrder.orderStatus) ||
          currentOrder.collectedAmount === null ||
          currentOrder.collectedAmount === undefined
        ) {
          results.push({
            orderIdRaw: row.orderIdRaw,
            success: false,
            reason: "এই অর্ডারে এখনো Collected Amount বসেনি",
          });
          continue;
        }

        const matched =
          Number(courierPaidAmount) === currentOrder.collectedAmount;
    ...
    for (const row of rows) {
      ...
      if (currentStatus === status) {
        results.push({
          orderIdRaw: row.orderIdRaw,
          success: true,
          reason: "আগে থেকেই এই স্ট্যাটাসে",
        });
        continue;
      }
      ...
      if (
        ...
        results.push({
          orderIdRaw: row.orderIdRaw,
          success: false,
          reason: `${status === "PAID_RETURN" ? "Paid Return" : "Delivered"}-এর জন্য Amount দরকার`,
        });
        continue;
      }
      ...
      if (
        ...
        results.push({
          orderIdRaw: row.orderIdRaw,
          success: false,
          reason: "CANCELLED স্ট্যাটাসে Amount দেওয়া যাবে না",
        });
        continue;
      }
      ...
      try {
        ...
        results.push({
          orderIdRaw: row.orderIdRaw,
          success: false,
          reason: msg.startsWith("STOCK_ERROR:")
            ? msg.replace("STOCK_ERROR:", "")
            : "সার্ভার সমস্যা",
        });
      }
    }
