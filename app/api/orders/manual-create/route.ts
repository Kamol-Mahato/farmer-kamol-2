...
export async function POST(request: Request) {
  ...
  try {
    ...
    const source = VALID_SOURCES.includes(orderSource)
      ? orderSource
      : "AGENT_MANUAL";
    ...
    for (const it of items) {
      ...
      if (!p) {
        return NextResponse.json(
          { error: "একটি প্রোডাক্ট খুঁজে পাওয়া যায়নি" },
          { status: 404 },
        );
      }
      ...
      if (p.stockQty < stockDeduction) {
        return NextResponse.json(
          {
            error: `${p.name}-এর পর্যাপ্ত স্টক নেই। উপলব্ধ: ${p.stockQty} কেজি`,
          },
          { status: 400 },
        );
      }
