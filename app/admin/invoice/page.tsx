      ...
      <table className="w-full text-sm mb-6 border-collapse">
        <thead>
          <tr className="bg-green-50 text-green-800">
            <th className="text-left px-3 py-2 font-bold border border-green-100">
              পণ্য
            </th>
            <th className="text-center px-3 py-2 font-bold border border-green-100">
              পরিমাণ
            </th>
            <th className="text-right px-3 py-2 font-bold border border-green-100">
              মূল্য
            </th>
          </tr>
        </thead>
...
function POSInvoice({ order, qrUrl }: { order: Order; qrUrl: string }) {
  ...
  return (
    <div
      className="invoice-container bg-white p-4 mb-6 border border-gray-200 rounded-xl mx-auto"
      style={{ width: "302px" }}
    >
      ...
      <p className="text-xs font-bold text-red-600">
        বাকি: ৳ {order.finalCodAmount - order.paymentAmountPaid}
      </p>
      <div className="border-t border-dashed border-gray-400 my-2" />
      <p className="text-xs">
        <span className="font-bold">নাম:</span> {order.customer.name}
      </p>
      <p className="text-xs mt-1">
        <span className="font-bold">ফোন:</span> {order.customer.phone}
      </p>
      <p className="text-xs mt-1">
        <span className="font-bold">ঠিকানা:</span> {order.deliveryAddress}
      </p>
      <div className="border-t border-dashed border-gray-400 my-2" />
      {order.orderItems.map((item) => (
        <p key={item.id} className="text-xs">
          {item.product.name} × {item.quantity} = ৳ {item.finalPrice}
        </p>
      ))}
      <div className="border-t border-dashed border-gray-400 my-2" />
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 pb-1 mb-1 border-b border-dashed border-gray-400">
        ...
        />
        <span className="font-extrabold text-green-800 text-xs">
          FARMER KAMOL
        </span>
      </div>
...
function InvoicePage() {
  ...
  const ordersBasePath = pathname?.startsWith("/agent")
    ? "/agent/orders"
    : "/admin/orders";
  ...
  useEffect(() => {
    async function loadAll() {
      const results = await Promise.all(
        ids.map((id) =>
          fetch(`/api/admin/invoice?id=${id}`).then((r) => r.json()),
        ),
      );
      setOrders(results.filter((o) => o && o.id));
      const qr = await QRCode.toDataURL(
        "https://www.youtube.com/@FarmerKamol",
        { width: 80, margin: 1 },
      );
      setQrUrl(qr);
      setLoading(false);
  ...
  if (loading)
    return <div className="text-center py-20 text-gray-400">লোড হচ্ছে...</div>;
  if (orders.length === 0)
    return (
      <div className="text-center py-20 text-red-400">অর্ডার পাওয়া যায়নি</div>
    );
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-4">
      <div className="max-w-4xl mx-auto mb-4 flex gap-3 print:hidden">
        ...
        <a
          href={ordersBasePath}
          className="ml-auto text-gray-500 hover:text-green-700 text-sm flex items-center"
        >
          ← ফিরে যান
        </a>
      </div>
