...
export default function AdminVideosPage() {
  ...
  async function addToHero(videoId: number) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        alert(data.error || "যোগ করা যায়নি");
        return;
      }
  ...
  if (loading)
    return (
      <div className="text-center py-20 text-gray-500">ভিডিও লোড হচ্ছে...</div>
    );
    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-8">
        <h2 className="text-lg font-bold text-green-700 mb-4">
          {editingId ? "ভিডিও এডিট করুন" : "নতুন ভিডিও যোগ করুন"}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-3">
            <label className="block text-xs font-semibold text-gray-500 mb-1">
              কাস্টম থাম্বনেইল (YT + FB — ক্লিক রেট বাড়াতে)
            </label>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              />
              {uploadingThumb && (
                <span className="text-xs text-gray-500">আপলোড হচ্ছে...</span>
              )}
              {thumbnailUrl && (
        ...
        <div className="mt-4 flex gap-3">
          <button
            onClick={handleSave}
            className="bg-green-700 text-white px-6 py-2 rounded-lg font-bold text-sm hover:bg-green-600 transition"
          >
        ...
        {videos.length === 0 ? (
          ...
          (showAll ? videos : videos.slice(0, INITIAL_LIMIT)).map((video) => {
            ...
            return (
              <div
                key={video.id}
                className={`bg-white rounded-xl shadow overflow-hidden border ${!video.isActive ? "opacity-50" : ""}`}
              >
                {video.thumbnailUrl ? (
                  <img
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {(() => {
                      const heroEntry = heroVideos.find(
                        (h) => h.id === video.id,
                      );
                      return heroEntry ? (
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-xs font-bold transition ${video.isActive ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-200" : "bg-green-100 text-green-700 hover:bg-green-200"}`}
                    >
                    ...
                    {video.platform === "YOUTUBE" &&
                      (() => {
                        ...
                        return (
                          <>
                            <button
                              onClick={() => reorderHero(video.id, "up")}
                              disabled={busy || heroEntry.heroOrder === 0}
                              className="bg-gray-100 text-gray-700 px-2 py-1 rounded-lg text-xs font-bold hover:bg-gray-200 transition disabled:opacity-30"
                            >
                              ▲
                            </button>
                            <button
                              onClick={() => reorderHero(video.id, "down")}
                              disabled={busy}
                              className="bg-gray-100 text-gray-700 px-2 py-1 rounded-lg text-xs font-bold hover:bg-gray-200 transition disabled:opacity-30"
                            >
                              ▼
                            ...
                            <button
                              onClick={() => removeFromHero(video.id)}
                              disabled={busy}
