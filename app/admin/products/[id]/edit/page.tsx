    ...
    fetch(`/api/admin/products/${id}`)
      ...
      .then((data) => {
        setForm({
          ...
          imageUrls:
            data.images?.map((img: { imageUrl: string }) => img.imageUrl) || [],
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (!res.ok) {
          setError(data.error || "ছবি আপলোড ব্যর্থ");
          continue;
        }
        setForm((prev) => ({
          ...prev,
    ...
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="bg-white rounded-xl shadow p-8">
          ...
          <div className="flex items-center gap-4">
            ...
            {form.imageUrls.length > 0 && (
              <span className="text-sm text-green-600 font-medium">
                ✅ {form.imageUrls.length} টি ছবি আছে
              </span>
            )}
          </div>
          {form.imageUrls.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-3">
              {form.imageUrls.map((url) => (
                <div
                  key={url}
                  className="relative w-28 h-28 rounded-xl border border-gray-200 overflow-hidden bg-gray-50"
                >
                  <img
                    src={url}
                    alt="preview"
        ...
        <div className="mb-6">
          ...
          >
            <option value="FIXED">
              নির্দিষ্ট দাম — ওয়েবসাইটে Add to Cart/অর্ডার বাটন থাকবে
            </option>
