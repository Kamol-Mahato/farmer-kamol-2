...
function OrderForm() {
  ...
  const base = isDhaka
    ...
    : deliverySettings.outsideBaseCharge;
  const extra = isDhaka
    ? deliverySettings.dhakaExtraPerUnit
    : deliverySettings.outsideExtraPerUnit;
  // আপনার আগের মূল হিসাবের লজিক হুবহু রাখা হয়েছে
  const regularDeliveryCharge =
    selectedDistrictId === null
      ? deliverySettings.dhakaBaseCharge
      : form.quantity > 1
        ? base + (Number(form.quantity) - 1) * extra
        : base;

  // ✅ নতুন মোড (FREE/HALF/NORMAL) অনুযায়ী ফাইনাল চার্জ ক্যালকুলেশন
  const deliveryCharge =
    deliverySettings.mode === "FREE"
      ? 0
    ...
    return (
      <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-200 mb-4 text-left">
        ...
        <p className="text-gray-500 text-xs mb-3">
          পরবর্তীতে অর্ডার ট্র্যাক করতে পাসওয়ার্ড দিন (ঐচ্ছিক)
        </p>
  ...
  async function handleSubmit(e: React.FormEvent) {
    if (!isValidBDPhone(form.phone)) {
      setError(
        "আপনার মোবাইল নম্বরটি সঠিক নয় (১১ ডিজিট হতে হবে এবং 01 দিয়ে শুরু হতে হবে)",
      );
      return;
    }
    e.preventDefault();
    ...
    if (product && product.stockQty < form.quantity) {
      setError(
        `দুঃখিত, পর্যাপ্ত স্টক নেই। উপলব্ধ স্টক: ${product.stockQty} টি`,
      );
      return;
    }
    ...
    try {
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          gatewayName:
            form.paymentMethod === "GATEWAY" ? form.gatewayName : null,
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md text-center">
          ...
          <div className="bg-green-50 rounded-xl p-4 border border-green-100 text-left mb-4">
            ...
            <p className="text-gray-500 text-xs">
              পরিমাণ: {form.quantity} × ৳ {product?.pricePerUnit}
            </p>
            <p className="text-gray-500 text-xs">
              ডেলিভারি: ৳ {deliveryCharge}
            </p>
            <p className="font-bold text-green-700 mt-2">
              মোট: ৳ {totalPrice + deliveryCharge}
            </p>
          </div>
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-6 h-6 bg-green-100 text-green-800 rounded-full text-xl font-bold hover:bg-green-200 flex items-center justify-center"
            >
              −
            </button>
            <input
              ...
              className="w-12 text-center border border-gray-200 rounded-lg px-1 py-2 text-sm focus:outline-none focus:border-green-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            />
            <button
              type="button"
              onClick={() =>
              ...
              className="w-6 h-6 bg-green-800 text-white rounded-full text-xl font-bold hover:bg-green-700 flex items-center justify-center"
            >
              +
            </button>
          </div>
          ...
          <div>
            ...
            {form.phone.length > 0 && !isValidBDPhone(form.phone) && (
              <p className="text-red-500 text-[10px] mt-1 font-bold">
                সঠিক ১১ ডিজিটের নম্বর দিন (01 দিয়ে শুরু)
              </p>
            )}
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <UpazilaSearch
              ...
              upazilas={
                selectedDistrictId ? upazilas[selectedDistrictId] || [] : []
              }
              upazilasEn={
                selectedDistrictId ? upazilasEn[selectedDistrictId] || [] : []
              }
              value={form.upazila}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              onClick={() =>
                setForm((f) => ({ ...f, paymentMethod: "GATEWAY" }))
              }
              className={`py-2 rounded-lg text-sm font-bold border-2 transition ${
                form.paymentMethod === "GATEWAY"
                  ? "border-green-600 bg-green-50 text-green-800"
            ...
            <div className="bg-white rounded-lg p-3 border border-gray-200 flex items-center justify-between">
              <div>
                ...
                <p className="font-bold text-gray-800 text-base">
                  {siteConfig.payment.bkashNumber}
                </p>
              </div>
              <button
                type="button"
