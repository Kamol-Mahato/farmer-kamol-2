...
export async function generateMetadata({
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/banglar-fosol/${item.category.slug}/${item.slug}`,
      languages: {
        bn: `/banglar-fosol/${item.category.slug}/${item.slug}`,
        en: item.slugEn
          ? `/en/banglar-fosol/${item.category.slug}/${item.slugEn}`
          : `/banglar-fosol/${item.category.slug}/${item.slug}`,
      },
    },
    openGraph: {
      title: item.title,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: "বাংলার ফসল",
        item: `${siteConfig.domain.url}/banglar-fosol`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: item.category.name,
        item: `${siteConfig.domain.url}/banglar-fosol/${item.category.slug}`,
      },
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 pt-8 md:pt-6">
        <nav className="text-sm text-gray-500 mb-4">
          ...
          <Link href="/banglar-fosol" className="hover:text-green-700">
            বাংলার ফসল
          </Link>
          <span className="mx-1.5">/</span>
          <Link
