...
// পেমেন্ট মেথডের রঙ ও জেনেরিক আইকন — কোনো ব্র্যান্ডের আসল লোগো/ডিজাইন কপি করা হয়নি (কপিরাইট সেফ)
const paymentIcons = [
  {
    // বিকাশ — জেনেরিক মোবাইল-ওয়ালেট আইকন
    bg: "bg-pink-600",
    svg: (
  ...
  {
    // কার্ড — জেনেরিক পেমেন্ট কার্ড আইকন
    bg: "bg-blue-600",
    svg: (
...
export default function Footer() {
  ...
  return (
    // ১. প্রধান ফুটার এরিয়া
    <footer className="bg-green-50 text-gray-700 mt-auto">
      <div className="max-w-7xl mx-auto px-4 pt-8 pb-6">

        {/* ব্র্যান্ড সেকশন — ফুল উইডথ, উপরের সারিতে */}
        <div className="mb-6 md:mb-8 pb-6 md:pb-8 border-b border-green-100">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div className="md:max-w-xl">
              <Link href={href("/")} className="flex items-center gap-2 mb-1">
                ...
                />
                <span className="text-lg font-bold text-green-900">{siteConfig.brand.name}</span>
              </Link>
              <p className="text-sm text-amber-700 mb-2 font-bold">{t.slogan}</p>
              <p className="text-sm text-black mb-4 md:mb-0">{t.description}</p>
            </div>

            {/* সোশ্যাল এবং ইমেইল আইকনের সারি — সবগুলো ফুল রাউন্ড শেপে */}
            <div className="flex flex-wrap gap-2 md:shrink-0">
              ...
              <a href={siteConfig.social.youtube} target="_blank" rel="noopener noreferrer" aria-label="YouTube"
                className="w-9 h-9 rounded-full bg-red-600 hover:bg-red-500 flex items-center justify-center transition">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="white">
              ...
              <a href={`https://wa.me/${siteConfig.contact.whatsapp}`} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"
                className="w-9 h-9 rounded-full bg-green-500 hover:bg-green-400 flex items-center justify-center transition">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="white">
              ...
              <a href={siteConfig.social.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                className="w-9 h-9 rounded-full bg-pink-600 hover:bg-pink-500 flex items-center justify-center transition">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
              ...
              <a href={siteConfig.social.tiktok} target="_blank" rel="noopener noreferrer" aria-label="TikTok"
                className="w-9 h-9 rounded-full bg-black hover:bg-gray-800 flex items-center justify-center transition">
                <span className="text-white text-xs font-bold">TT</span>
              </a>

              {/* ইমেইল বাটন */}
              <a href={`mailto:${siteConfig.contact.email}`} aria-label="Email Us"
                className="w-9 h-9 rounded-full bg-emerald-600 hover:bg-emerald-500 flex items-center justify-center transition">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* লিংক সেকশন — Mobile এ ৩ কলাম (পণ্য, সেবা, পেমেন্ট) এক লাইনে, Desktop এ ৪ কলাম বরাবর */}
        <div className="grid grid-cols-3 md:grid-cols-4 gap-4 md:gap-6 items-start">
          ...
          {/* Payment Methods — 2x2 গ্রিড, ফুল রাউন্ড, জেনেরিক আইকন (কপিরাইট সেফ) */}
          <div>
            <h4 className="font-bold mb-1 text-sm text-green-900">{t.paymentHeading}</h4>
            <p className="text-xs text-gray-500 mb-3">{t.weAccepted}</p>
            <div className="grid grid-cols-2 gap-3">
              {paymentIcons.map((p, i) => (
                <div key={t.paymentMethods[i]} className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full ${p.bg} flex items-center justify-center shrink-0`}>
                    {p.svg}
                  </div>
                  <span className="text-sm text-black">{t.paymentMethods[i]}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Our Farm — Mobile এ ফুল উইডথ + সেন্টার, Desktop এ ৪র্থ কলাম হিসেবে বরাবর */}
          <div className="col-span-3 md:col-span-1">
            ...
            <div className="flex flex-col gap-1.5 text-sm text-black items-center md:items-start">
              ...
              <a href="https://wa.me/8801521406139" target="_blank" rel="noopener noreferrer" className="hover:text-green-700 transition">💼 {t.businessContact}: 01521406139</a>
              <a href={`mailto:${siteConfig.contact.email}`} className="hover:text-green-700 transition break-all">✉️ {siteConfig.contact.email}</a>
            </div>
          </div>
        </div>
