...
const typeLabel: Record<SearchResult["type"], { bn: string; en: string; emoji: string }> = {
...
export default function GlobalSearch({ variant, onClose, autoFocus }: Props) {
  ...
  return (
    <div ref={wrapRef} className={`relative ${variant === "mobile" ? "w-full" : ""}`}>
      ...
      {showDropdown && (
        <div
          className={`absolute z-[100] mt-1 bg-white text-gray-900 rounded-xl shadow-xl border border-gray-200 overflow-hidden ${
            variant === "navbar" ? "right-0 w-72 md:w-80" : "left-0 right-0 w-full mt-2"
          }`}
        >
          {showHistory && (
            <div className="p-2">
              <div className="flex items-center justify-between px-2 py-1">
                <span className="text-xs font-semibold text-gray-500">{t.recent}</span>
                <button type="button" onClick={handleClearHistory} className="text-[11px] text-red-500 hover:underline">
                  {t.clear}
                </button>
              </div>
              {history.map((h) => (
          ...
          {query.trim().length > 0 && (
            <div className="max-h-72 overflow-y-auto">
              ...
              {!loading && results.length === 0 && (
                <div className="px-4 py-3 text-sm text-gray-500">{t.noResult}</div>
              )}
              {!loading &&
                results.map((item) => {
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-[11px] text-gray-500 flex items-center gap-1.5">
                          <span>
                            {label.emoji} {locale === "bn" ? label.bn : label.en}
                          </span>
                          {item.subtitle && (
                            <>
                              <span>·</span>
